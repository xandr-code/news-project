...
public class ArticleParser {
    ...
    private List<NewsCandidate> parseFeed(Elements items, String sourceUrl) {
        ...
        for (Element item : items) {
            ...
            if (!title.isBlank() && !url.isBlank()) {
                candidates.add(new NewsCandidate(title, absolutize(sourceUrl, url), description, content, mediaUrl, publishedAt));
            }
        }
        return candidates;
    }
    ...
    private NewsCandidate parseHtmlPage(String url) throws IOException {
        ...
        return new NewsCandidate(title, canonical, description, content, mediaUrl, LocalDateTime.now());
    }
